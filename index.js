    ...
    for (let game of games) {
        ...
        gameCard.innerHTML = `
            ...
            ${
                ...
                ? `<p style="color: red";>Funds Needed: $${(game.goal - game.pledged).toLocaleString('en-US')}</p>` 
...
function filterUnfundedOnly() {
    ...
    console.log("Under Funded Games:", unfundedGames.length);
...
const message = `A total of $${totalMoneyRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} game${GAMES_JSON.length > 1 ? 's' : ''}. There ${unfundedGamesCount === 1 ? 'is' : 'are'} ${unfundedGamesCount} unfunded game${unfundedGamesCount === 1 ? '' : 's'} remaining.`;
